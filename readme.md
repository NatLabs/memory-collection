...
- [MemoryCmp](./src/TypeUtils/MemoryCmp.mo): A module that provides functions for comparing two elements of the same type and retrieving their order. The comparison function is a `#BlobCmp` which compares the serialized version of the types directly as `Blob`s.

  > **Deprecated:** `#GenCmp` (which deserialized blobs back to their original type before comparing) has been removed. Use `#BlobCmp` with an order-preserving blob encoding instead.

```motoko
  public type MemoryCmp<T> = {
    // Deprecated and removed: use #BlobCmp with an order-preserving Blobify instead.
    // #GenCmp: (T, T) -> Int8;
