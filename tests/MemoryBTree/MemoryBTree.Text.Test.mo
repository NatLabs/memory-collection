...
import BTreeUtils "MemoryBTreeConfig/Utils";
...
func btree_tests(node_capacity : Nat, data : Buffer.Buffer<(Text, Text)>, sorted : Buffer.Buffer<(Text, Text)>, test_name : Text) {
                ...
                func() {
                    ...
                    for ((i, (key, prev_val)) in Itertools.enumerate(data.vals())) {
                        ...
                        if (exists == null) {
                            ...
                            Runtime.trap("Key not found BEFORE remove: i=" # Nat.toText(i) # " key=\"" # key # "\"");
                        } else {
...
suite(
    ...
    func() {
        for (node_capacity in [32, 256].vals()) {
            btree_tests(node_capacity, random_keys, random_sorted, "Random cap=" # Nat.toText(node_capacity));
        };
    },
);
...
suite(
    ...
    func() {
        for (node_capacity in [16, 32, 256].vals()) {
            btree_tests(node_capacity, prefixed_keys, prefixed_sorted, "Prefixed cap=" # Nat.toText(node_capacity));
        };
    },
);

// Benchmark-replicating test: uses BTreeUtils.generate_prefixed_text_keys
// (PREFIX_REUSE_RATE=99, MAX_PREFIX_LEN=60) with same seed as the node-capacity benchmark.
// This exercises the split path that corrupted left-leaf prefixes when the left
// separator differed from old_prefix at a byte within the old_prefix range.
suite(
    "Benchmark-Replica Tests (BTreeUtils, PREFIX_REUSE_RATE=99)",
    func() {
        let bench_fuzz = Fuzz.fromSeed(0xdeadbeef);
        let bench_data = BTreeUtils.generate_prefixed_text_keys(bench_fuzz, 10_000);
        let bench_sorted = sort_text_buffer(bench_data);

        for (node_capacity in [16, 32, 64].vals()) {
            btree_tests(node_capacity, bench_data, bench_sorted,
                "BenchData cap=" # Nat.toText(node_capacity) # " PC=ON");
        };
    },
);
